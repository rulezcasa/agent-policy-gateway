...
function Sidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return <aside className="flex h-full w-64 flex-col border-r border-slate-200/80 bg-white px-3 py-5 shadow-[1px_0_0_rgba(15,23,42,0.02)]">
    <Link href="/dashboard" onClick={onNavigate} className="flex items-center gap-3 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500">
      <span className="flex size-9 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-700 text-[11px] font-bold tracking-tight text-white shadow-sm shadow-indigo-200">AP</span>
      <span className="leading-tight"><span className="block text-sm font-semibold tracking-tight text-slate-950">Agent Policy</span><span className="block text-xs font-medium text-slate-500">Gateway</span></span>
    </Link>
    <nav aria-label="Primary navigation" className="mt-9 flex flex-1 flex-col gap-1.5">
      {navigation.map((item) => {
        const active = isActive(item, pathname);
        return <Link key={item.href} href={item.href} onClick={onNavigate} className={`relative flex items-center gap-3 rounded-lg px-3 py-2.5 text-[14px] font-medium transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-500 ${item.href.startsWith("/policies/") ? "ml-3" : ""} ${active ? "bg-indigo-50/70 text-indigo-700 before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:rounded-full before:bg-indigo-600" : "text-slate-500 hover:bg-slate-100/70 hover:text-slate-900"}`}><Icon name={item.icon} />{item.label}</Link>;
      })}
    </nav>
    <div className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-3.5"><div className="flex items-center gap-2"><span className="size-1.5 rounded-full bg-emerald-500" /><p className="text-xs font-semibold text-slate-700">Policy enforcement</p></div><p className="mt-2 text-xs leading-5 text-slate-500">Human oversight for every consequential action.</p></div>
  </aside>;
}
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const title = pageTitles[pathname] ?? "Agent Policy Gateway";
  return <div className="min-h-screen bg-[#f6f7fb] text-slate-950">
    ...
    <div className="lg:pl-64">
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
        <div className="flex items-center gap-3"><button type="button" aria-label="Open navigation" className="rounded-lg p-2 text-slate-600 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 lg:hidden" onClick={() => setMobileNavOpen(true)}><Icon name="menu" /></button><h1 className="text-[19px] font-semibold tracking-[-0.025em] text-slate-950">{title}</h1></div>
        <div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-semibold text-slate-800">Admin</p><p className="text-xs text-slate-500">Policy administrator</p></div><div aria-label="Admin account" className="flex size-9 items-center justify-center rounded-full bg-gradient-to-br from-slate-700 to-slate-900 text-xs font-semibold text-white ring-2 ring-slate-100">AD</div></div>
      </header>
